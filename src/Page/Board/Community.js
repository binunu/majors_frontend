...
const Community = () => {
    ...
    const getArticles=(category,page)=>{ 
      ...
      axiosURL.get(`/board/article/list/community/${category}/${page}`)
        .then(res => {  
  ...
  return (
    <div id='study-as-list' className='main-board'>
      {render &&
        <> 
          <MajorSelect type={"자유게시판"} cateInit={major} changeMajor={changeMajor} />
          <div className='mode'>
            <div>
              <Link to={`/boardAsPeed/community/${major}/1`}><PeedIcon className='icon' /></Link>
